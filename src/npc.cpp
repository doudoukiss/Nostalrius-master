...
Npc* Npc::createNpc(const std::string& name, bool lua)
{
	...
	if (!lua) {
		...
		npc->filename = "data/npc/lua/" + name + ".xml";
	}
...
bool Npc::load()
{
	...
	if (isLuaNpc) {
		if (!scriptInterface) {
			...
			scriptInterface->loadNpcLib("data/npc/lua/lib/npc.lua");
		}
...
{
	if (creature->getID() == id || type != TALKTYPE_SAY) {
		return;
	}
	...
	if (player) {
		if (isLuaNpc) {
			...
			if (!behaviourDatabase || !Position::areInRange<3, 3>(creature->getPosition(), getPosition())) {
				return;
			}
...
{
	loaded = scriptInterface->loadFile("data/npc/lua/scripts/" + file, npc) == 0;
	if (!loaded) {
